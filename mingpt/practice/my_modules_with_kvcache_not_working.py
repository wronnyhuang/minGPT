...
class Attention(nn.Module):
  ...
  def forward(self, x, kv_cache=None):
    ...
    mask = torch.tril(torch.ones(t_k, t_k))[t_k-t_q:, :]
    logits = torch.where(mask == 1, logits, -float('inf')) 
...
class Block(nn.Module):
  ...
  def forward(self, x, kv_cache=None):
    if kv_cache is not None:
      _x, kv_cache = self.attn(self.ln1(x), kv_cache)
      x = x + _x
...
class Transformer(nn.Module):
  ...
  def forward(self, idx, targets=None, kv_caches=None):
    ...
    for i, layer in enumerate(self.layers):
      if kv_caches is not None:
        kv_cache = kv_caches[i] if kv_caches else ()
  ...
  @torch.no_grad()
  def generate_kv(self, idx, max_new_tokens, do_sample=False):
    _, _, kv_caches = self(idx[:, :-1], kv_caches=[])
    curr_idx = idx[:, [-1]]
    for _ in range(max_new_tokens):
      logits, _, kv_caches = self(curr_idx, kv_caches=kv_caches)
      probs = F.softmax(logits[:, 0, :], dim=-1)
      _, curr_idx = torch.topk(probs, k=1, dim=-1)
      idx = torch.cat([idx, curr_idx], dim=-1)
    return idx
